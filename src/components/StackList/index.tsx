...
export default function StackList(props: StackListProps) {
  ...
  return <Box className={`StackList bg-base p-5 flex ${direction === 'column' ? 'fd-c' : ''} ${classNames.container ?? ''}`}>
    ...
    <Box className={`StackList-content bg-dark p-5 flex ${direction === 'column' ? 'pb-0' : 'pr-0'} ${direction === 'column' ? 'fd-c' : ''} ${classNames.content ?? ''}`}>
      {items.map(([label, value], index) => <Box className={`StackList-content-item flex jc-sb p-5 bg-base p-5 pr-0 ${direction === 'column' ? 'mb-5' : 'mr-5'} ${classNames.contentItem ?? ''}`} key={index}>
