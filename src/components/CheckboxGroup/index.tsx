...
export default function CheckboxGroup<I extends Record<string, any>>(props: CheckboxGroupProps<I>) {
  ...
  return <FormGroup className={`CheckboxGroup flex flex-col-1 ${classNames.formGroup ?? ''}`}>
    <InputLabel className={`${classNames.inputLabel ?? ''} text-lg`}>
      {props.label}
    </InputLabel>
    <FlexContent className={`CheckboxGroup-content flex flex-col-1 ${classNames.content ?? ''}`} direction="column">
