...
>(props: ListProps<T>) {
  ...
  return (
    <Container className={`List select-none flex-col-1 ${className}`}>
      <Header
        header={header}
        sideElements={[
          <Flex>
            <Hovertips
              popoverText={`${isAllSelected ? 'Deselect' : 'Select'} all items`}
            >
              <Checkbox
                color="primary"
                key={'checkbox'}
          ...
          <Flex>
            <Hovertips
              popoverText={`Remove ${selectedItems.length} selected items`}
              key={'delete icon'}
            >
              <MdDelete
                size={20}
                className={'List-header-icons-cancel'}
                fill={red[500]}
                onClick={() => {
                  remove();
                  ...
                  props.onDelete &&
                    props.onDelete(remainingItems, selectedItems);
                  setSelectedItems([]);
                }}
              />
            </Hovertips>
      ...
      <Content className={`h-full flex-col-1`}>
        {items.length > 0 ? (
          items.map((item, index) => {
            ...
            return (
              <Box
                className={`List-content-item flex items-center p-1.5 rounded-sm ${
                  index % 2 === 0 ? 'bg-light' : 'bg-base'
                }`}
                key={_id}
              >
                <Box className="List-content-item-icons p-1 flex items-center">
                  <Checkbox
                    color="primary"
                    className="List-content-item-icons-checkbox"
                    key={_id + 'checkbox' + index}
                    onClick={(e: any) => {
                      e.persist();
                      if (settings.sound) {
                  ...
                  >
                    <MdDelete
                      size={20}
                      className="List-content-item-icons-cancel"
                      fill={red[500]}
                      onClick={() => {
                        remove();
                        const remainingItems = items.filter(
                        ...
                        props.onDelete &&
                          props.onDelete(remainingItems, [item._id]);
                        setItems(remainingItems);
                        setSelectedItems(
                ...
                <Box className="flex flex-1 justify-between p-1 ">
                  {fields.map((field, index) => (
                    <Box
                      className="List-content-item-field flex items-center"
                      key={_id + field + index}
                    >
                      <Typography
                        variant="body1"
          ...
          <Box className="center text-center flex items-center justify-center bold p-1">
            <Typography variant="h5">{emptyListMessage}</Typography>
          </Box>
        )}
      </Content>
    </Container>
