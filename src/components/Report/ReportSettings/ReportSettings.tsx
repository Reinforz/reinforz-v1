...
export function ReportSettings() {
  ...
  return <div className="Report-Settings">
    {!reportFilter.excluded_columns.includes('play_options') ? <StackList header={"Play Options"} items={Object.entries(playSettings.options).map(([key, value]) => [transformTextBySeparator(key), <span style={{ color: value === true ? green[500] : red[500] }}>{value === true ? 'On' : 'Off'}</span>])} /> : null}
