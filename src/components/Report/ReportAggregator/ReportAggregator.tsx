...
export function ReportAggregator(props: Props) {
  ...
  const totalWeight = props.filteredResults.reduce((acc, filteredResult) => acc + filteredResult.question.weight, 0);
  ...
  return <div className="Report-Aggregator" style={{ color: theme.palette.text.primary }}>
    {([['Time Allocated', 'time_allocated'], ['Weight', 'weight']] as const).map(key => <div className="Report-Aggregator-item" key={key[0]}>
      <Select onChange={(e) => {
        localStorage.setItem('REPORT_AGGREGATOR', JSON.stringify({
          ...reportAggregator,
          [key[1]]: e.target.value
        }))
      }} menuItemLabel={(item) => item} label={key[0]} items={["MAX", "MIN", "AVG"]} setState={setReportAggregator} stateKey={key[1]} state={reportAggregator} />
      <div className="Report-Aggregator-item-value" style={{ backgroundColor: theme.color.light }}>{computeNumberDataAggregation(props.filteredResults.map(filteredResult => filteredResult.question[key[1]]), { aggregation: reportAggregator[key[1]] })}</div>
    </div>)}
    {([['Time Taken', 'time_taken'], ['Hints Used', 'hints_used']] as const).map(key => <div className="Report-Aggregator-item" key={key[0]}>
      ...
      <div className="Report-Aggregator-item-value" style={{ backgroundColor: theme.color.light }}>{computeNumberDataAggregation(props.filteredResults.map(filteredResult => filteredResult[key[1]]), { aggregation: reportAggregator[key[1]] })}</div>
    </div>)}
    <div className="Report-Aggregator-item">
      ...
      }} menuItemLabel={(item) => item} label={'Score'} items={["MAX", "MIN", "AVG"]} setState={setReportAggregator} stateKey={"score"} state={reportAggregator} />
    ...
    <div className="Report-Aggregator-item">
      ...
      }} menuItemLabel={(item) => item} label={"Verdict"} items={["TRUE", "FALSE"]} setState={setReportAggregator} stateKey={'verdict'} state={reportAggregator} />
