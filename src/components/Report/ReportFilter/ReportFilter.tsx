...
export default function ReportFilter(props: Props) {
  ...
  return <div className="ReportFilter" style={{ backgroundColor: theme.color.dark }}>
    ...
    <RadioGroup onClick={(e: any) => {
      localStorage.setItem('REPORT_FILTERS', JSON.stringify({
        ...reportFilter,
        verdict: e.target.value
      }))
    }} items={["true", "false", "mixed"]} label={"Verdict"} setState={setReportFilter} state={reportFilter} stateKey={"verdict"} />
    <RadioGroup onClick={(e: any) => {
      localStorage.setItem('REPORT_FILTERS', JSON.stringify({
        ...reportFilter,
        hints_used: e.target.value
      }))
    }} items={["0", "1", "2", "any"]} label={"Hints Used"} setState={setReportFilter} state={reportFilter} stateKey={"hints_used"} />
    <CheckboxGroup onChange={(items) => {
      localStorage.setItem('REPORT_FILTERS', JSON.stringify({
        ...reportFilter,
        excluded_difficulty: items
      }))
    }} label={'Excluded Difficulty'} items={['Beginner', 'Intermediate', 'Advanced']} setState={setReportFilter} stateKey={'excluded_difficulty'} state={reportFilter} />
    <CheckboxGroup onChange={(items) => {
      localStorage.setItem('REPORT_FILTERS', JSON.stringify({
        ...reportFilter,
        excluded_types: items
      }))
    }} label={'Excluded Type'} items={['FIB', 'MS', 'MCQ', "Snippet"]} setState={setReportFilter} stateKey={'excluded_types'} state={reportFilter} />
    ...
    <Select onChange={(e) => {
      localStorage.setItem('REPORT_FILTERS', JSON.stringify({
        ...reportFilter,
        excluded_columns: e.target.value
      }))
    }} multiple label={"Excluded Columns"}
