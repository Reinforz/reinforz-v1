...
export default function Select<T extends Record<string, any>>(props: SelectProps<T>) {
    ...
    <Box className={`Select-content flex flex-col bg-light p-1 ${classNames.content ?? ""}`}>
      <MuiSelect className={`${classNames.select ?? ""}`} value={state[stateKey] as string[]} multiple={multiple} renderValue={(items) => renderValue ? renderValue(items) : multiple ? (items as string[]).map((item) => transformTextBySeparator(item)).join(", ") : transformTextBySeparator(items.join("") as string) as ReactNode} onChange={(e) => {
            click();
