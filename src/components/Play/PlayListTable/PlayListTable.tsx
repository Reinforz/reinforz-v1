...
export function PlayListTable() {
  ...
  return <div className="PlayListTable">
    ...
    <ListTable className="PlayListTable--time_allocated" generateTitle={(item) => `${item.subject} - ${item.topic}`} headers={["15", "30", "45", "60", "90", "120"]} itemKey={"questions"} itemKeyKey={"time_allocated"} items={filteredQuizzes} itemMapKey={(item) => divideTimeAllocated(item)} />
  </div>
}