...
export function ReportOptions(props: Props) {
  ...
  return <Box className={`Report-Options bg-base mb-5 p-5 pb-0 flex-1 ${props.className ?? ''}`}>
    {question.options.map((option, answerIndex) => {
      return <Box className="Report-Options-container mb-5 p-5 bg-dark pb-0 flex fd-c" key={option.text}>
        <Box className="Report-Options-container-item flex jc-sb mb-5 jc-sb ai-c">
          ...
          {option.isCorrect ? <Box className="bg-light flex ai-c jc-c p-5 ml-5"><AiFillCheckSquare fill={green[500]} style={{ padding: 5 }} size={17.5} /></Box> : null}
          {option.userSelected ? <Box className="bg-light flex ai-c jc-c p-5 ml-5"><FaUser fill={theme.palette.text.primary} style={{ padding: 5 }} size={17.5} /> </Box> : null}
        </Box>
