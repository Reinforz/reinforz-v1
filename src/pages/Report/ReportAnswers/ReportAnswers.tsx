  ...
  return <Box className={`Report-Answers bg-base p-5 flex-1 mb-5 pb-0 ${props.className ?? ''}`}>
    {
      question.answers.map((answers, answerIndex) => {
        return <Box className="Report-Answers-container mb-5 p-5 pb-0 flex flex-col bg-dark" key={answerIndex}>
          <Box className="flex mb-5">
            <Typography className="flex p-10 justify-center items-center bold fs-18 mr-5">{answerIndex + 1}</Typography>
            <Typography className="Report-Answers-container-userAnswer bg-light p-5 bold flex justify-center items-center text-center fs-18 flex-1">{userAnswers[answerIndex] ?? 'N/A'}</Typography>
          </Box>
          {answers.map((answer, alternateIndex) => <Box className="Report-Answers-container-item pb-0 mb-5 bg-base p-5" key={alternateIndex}>
            <Box className="mb-5 flex justify-between">
              <Typography className="flex p-10 justify-center items-center bold fs-16 mr-5">{alternateIndex + 1}</Typography>
              ...
              {answer.isCorrect && <Box style={{ width: 50 }} className="Report-Answers-container-item-isCorrect bg-light flex items-center ml-5 justify-center"> <AiFillCheckSquare fill={green[500]} style={{ padding: 5 }} size={20} /></Box>}
            </Box>
