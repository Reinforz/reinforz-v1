...
function ReportTableRow(props: { result: IResult, reportQuestion: ReactNode, index: number }) {
  const { settings, theme } = useThemeSettings();
  const { setModalState } = useContext(ModalContext);
  const { excludedColumns, setReport, report } = useContext(ReportContext);
  const { result, reportQuestion, index } = props;

  const showHints = result.question.hints.length !== 0 && !excludedColumns['hints'];
  return <div key={result.question._id} className="Report-Table-item pb-0 p-5 bg-dark">
    <div className="flex ai-c jc-c bg-dark mb-5">
      ...
      <div className="Report-Table-item-modal flex jc-sb c-p ai-c" style={{ width: 40 }}><Hovertips popoverText="View separate"><BsFilm fill={theme.color.opposite_dark} size={15} onClick={() => {
        setModalState([true, <div>Hello World</div>])
      }} /></Hovertips>
        <Hovertips popoverText="Delete"><MdDelete size={20} fill={red[500]} onClick={() => {
          settings.sound && sounds.remove.play();
          setReport({
            ...report,
            results: report.results.filter(_result => _result._id !== result._id)
          })
        }} /></Hovertips></div>
    </div>
    {reportQuestion}
    ...
    <div className="flex">
      ...
      {showHints ? <div className="Report-Table-item-hints bg-base p-5 mb-5" style={{ width: '25%' }}>
        {result.question.hints.map(hint => <div className="Report-Table-item-hints-item bg-light p-5 mb-5" key={hint}>
...
export function ReportTable() {
  ...
  return <div className="Report-Table p-5 bg-base overflowY-auto">
    {sortedResults.map((sortedResult, index) => <ReportTableRow index={index} result={sortedResult} reportQuestion={memoizedReportQuestions[index]} />)}
  </div>
}